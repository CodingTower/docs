...
description: "Optimize your apps for search engines and AI systems with sitemaps, meta tags, structured data, Open Graph images, and performance tuning."
---
## How SEO and GEO work in ZenOpus

ZenOpus gives you a lot of flexibility: **Search Engine Optimization (SEO)** and **Generative Engine Optimization (GEO)** emerge from the code the ZenOpus Agent generates when you create pages, content, and features.
...
ZenOpus also includes a **built-in Speed tool powered by Google Lighthouse**. It provides a quick snapshot of your published site’s performance, accessibility, best practices, and SEO checks.

![ZenOpus Speed Tool](/images/zenopus/tips-tricks/seo-geo/speed-tool.png)
...
## Understanding ZenOpus’s architecture

ZenOpus builds modern web applications using React and Vite, meaning your app is a **client-side rendered (CSR)** **single-page application (SPA)**. Instead of serving a different HTML file for every page, the browser loads a small initial HTML shell and then handles routing and rendering with JavaScript.

**Traditional platforms** like WordPress or Webflow send the browser a fully assembled HTML page for each URL.

**ZenOpus** sends code and instructions that your device uses to build each view. All “pages” are really just different states of the same application.
...
1. **Return later to render JavaScript (JS)** and capture the full content
...
Follow the best practices in this guide to **ensure your ZenOpus site is optimized for search engines, social platforms, and AI systems and can compete effectively.**

## Foundation: Make your site crawlable

These steps ensure search engines can access, discover, and index all the important pages of your ZenOpus app.

### XML sitemap

Sitemaps help search engines discover all your pages. This is especially important for CSR sites where crawlers can’t easily find all routes. ZenOpus Agent can generate `sitemap.xml` with all public routes when requested.
...
`robots.txt` tells search engines which areas of your site they’re allowed to crawl. It’s important to avoid blocking key resources that Google needs to render your pages correctly. ZenOpus Agent can create a `robots.txt` file when requested.
...
Canonical tags tell search engines which URL is the “official” version of a page, helping prevent duplicate content issues. ZenOpus Agent can generate canonical tags when requested.
...
Search engines rely on URLs to understand what a page is about, so clean and descriptive URLs help improve both crawlability and rankings. ZenOpus Agent typically generates clean routes automatically through React Router.
...
ZenOpus Agent can create nav/footer links and suggest or create contextual links when requested.
...
Page titles are a critical on-page ranking factor. They show up in search results and browser tabs, and they have a strong impact on click-through rates. ZenOpus Agent can generate title tags based on your page’s purpose.
...
Meta descriptions appear in search results just below the page title. They are not a direct ranking factor, but they have a strong influence on click-through rates. ZenOpus Agent can add meta descriptions when requested.
...
A clear heading structure (H1 to H3) helps search engines understand how your content is organized and which sections are most important. ZenOpus Agent can generate proper H1 tags and a logical hierarchy when requested.
...
Semantic HTML uses meaningful tags to describe your content, helping both search engines and screen readers understand the structure of your page. ZenOpus Agent typically adds semantic elements such as `<main>`, `<nav>`, and `<section>`.
...
Backlinks help your ZenOpus site:
...
1. **Get listed in directories and resource pages**
...
Structured data, implemented through JSON-LD schema, gives search engines additional context about your content and can enable rich results like star ratings, product details, and FAQ dropdowns. ZenOpus Agent can generate schema markup when requested.
...
ZenOpus Agent includes basic OG and Twitter metadata by default, but **you need to customize the image, title, and description for each page.**
...
Page speed is a confirmed ranking factor. Faster sites rank higher and convert better. ZenOpus Agent uses modern optimization patterns like lazy loading and code minification to support fast rendering.

- Use ZenOpus’s **built-in Speed tool** (powered by Google Lighthouse) to analyze your site’s performance, accessibility, best practices, and SEO checks.
...
- Run **Speed** tool in ZenOpus (Google Lighthouse audit)
- Target scores:
    - Performance: 90+
    - Accessibility: 90+
    - Best Practices: 90+
...
More than 60% of searches happen on mobile. Google uses mobile-first indexing, which means your mobile version determines rankings. ZenOpus Agent produces responsive layouts with `viewport` tag. The `viewport` tag is required for mobile-friendly scaling.
...
- **Confirm the** `viewport` **tag exists.** Paste this in the browser console:and you should see:
    ```text
...
AI crawlers read HTML directly without executing JavaScript. Clear structure and schema help them extract facts for citations. ZenOpus Agent provides semantic structure and can add schema when requested.
...
Controls which AI systems can access your content for training or answering queries. ZenOpus Agent can configure the `robots.txt` to allow desired bots when requested.
...
AI systems prefer content that is clear, structured, and easy to quote. Providing information in this format makes it more likely to be cited accurately. ZenOpus Agent can generate FAQ sections and structured content when requested.
...
A dedicated summary page makes it much easier for AI systems to understand and cite your company’s key information. ZenOpus Agent can build dedicated summary pages with key information in crawlable format when requested.
...
- **Meta tag verification**: prompt ZenOpus to add the meta tag

> Add GSC verification meta tag: &lt;meta name=‘google-site-verification’ content=‘YOUR_CODE’ /&gt; to the &lt;head&gt;

- **HTML file upload**: upload Google’s verification file and ask ZenOpus to place it at the site root (usually `/public`)

**Submit sitemap**

1. In GSC, go to **Sitemaps**
1. Enter your sitemap URL: `https://yourdomain.com/sitemap.xml`
1. Click **Submit**
...
1. Open **View Tested Page** to check:
    - Screenshot of what Googlebot sees
    - Rendered HTML
    - Console errors
    - Blocked resources
1. Click **Request Indexing** for new or updated pages (rate limited)
...
<AccordionGroup>
<Accordion title="Can my site rank well on search using ZenOpus?">
Yes. ZenOpus apps can rank like any modern JavaScript site. As long as your content loads correctly and key resources aren’t blocked, Google can crawl, render, index, and rank your pages. Examples performing well today:
...
<Note>
Search performance also depends on factors outside ZenOpus’s control, including content quality, backlinks, competition, and search ranking algorithms.
</Note>
</Accordion>
<Accordion title="How can I get the best SEO and GEO results with ZenOpus?">
To get strong SEO and GEO results with ZenOpus, follow the best practices described in this guide. Make sure you cover these essentials:
...
<Accordion title="When is ZenOpus a good fit for SEO and GEO, and when should I consider SSR or prerendering?">
ZenOpus works well for many SEO and GEO scenarios, especially small to medium sites where you can manage metadata, internal links, and sitemaps with intention.
...
- Performance and mobile optimization are straightforward on ZenOpus
...
For some large or highly competitive projects, server-rendered or prerendered pages can complement ZenOpus by improving crawl efficiency and preview reliability.
...
A `zenopus.app` subdomain works well for:
...
<Accordion title="Can AI search engines (Perplexity, ChatGPT, Gemini, Claude) index ZenOpus apps?">
...
ZenOpus doesn’t include prerendering out of the box, but you can add it through your hosting or proxy using external services such as:
...
→ Ask ZenOpus to install `react-helmet-async` and set unique titles and descriptions for each page.
</Accordion>
</AccordionGroup>
