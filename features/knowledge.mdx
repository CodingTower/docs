---
title: "Define workspace and project knowledge"
description: "Define persistent instructions for ZenOpus using workspace knowledge and project knowledge. Set shared coding standards, architecture rules, and project context that ZenOpus remembers across conversations and projects."
---
Knowledge lets you provide persistent instructions and context to the ZenOpus agent. Instead of repeating the same explanations in every conversation, you can define them once and ZenOpus will consider them when generating edits.
...
Both are plain text fields that provide persistent instructions to the ZenOpus agent.
...
- Things ZenOpus should avoid doing
...
If you update workspace knowledge during an active conversation, ZenOpus will use the updated instructions on follow-up messages.
...
If a project needs different behavior from the workspace defaults, you can define those rules in **project knowledge**. When the same instruction appears in both places, ZenOpus will generally prioritize **project knowledge**, since it is more specific to the current project.
</Accordion>
...
Imagine explaining the project to a new developer joining the team. Include the important architectural decisions so ZenOpus does not attempt to change them.
</Accordion>
...
If your stack or architecture changes, update your knowledge so ZenOpus does not follow outdated patterns.
...
Yes. Instruction files such as `AGENTS.md` or `CLAUDE.md` can also provide guidance to the ZenOpus agent.

For technical users managing instruction files directly in their repositories, root-level `AGENTS.md` files are always read by the ZenOpus agent regardless of session length.
</Accordion>
<Accordion title="Which context sources does ZenOpus read?">
When you send a message, ZenOpus reads your **project knowledge**, **workspace knowledge**, and **project code** to understand how your project works before generating edits.
...
Project knowledge and workspace knowledge are included together when ZenOpus generates responses.

If the instructions conflict, ZenOpus is encouraged to prioritize the instructions defined in **project knowledge**, since they apply specifically to the current project.
