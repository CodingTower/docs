---
title: "Test and verify your app"
description: "ZenOpus provides browser testing, frontend tests, and edge function verification to help understand system behavior, validate user workflows, and ensure backend logic works as expected."
---
ZenOpus gives you several ways to check that your application behaves as expected. These tools help you reproduce issues, confirm fixes, and avoid regressions across frontend and backend behavior.
...
## What ZenOpus can observe during verification

ZenOpus collects information when the agent runs verification tools. This information is gathered automatically as part of those tools and is used to understand what is happening in your app.
...
ZenOpus supports several testing approaches, each designed for a different kind of verification. Most tests fall into one of two categories:
...
ZenOpus provides two complementary ways to verify backend behavior in edge functions: direct calls and automated tests. These approaches are often used together.
...
1. Apply the fix once the behavior is understood.
1. Call the function again with the same input to confirm the change.
1. Add an edge test to ensure the behavior does not regress over time.
...
When an issue does not resolve after a few attempts, you can ask ZenOpus to use multiple verification tools. ZenOpus runs these tools and uses their output to guide the next step.
...
<Accordion title="Does ZenOpus run verification tools automatically?">
