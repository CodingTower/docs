...
description: "Agent mode lets ZenOpus implement and verify changes directly in your project, handling execution end to end with minimal supervision."
---
<Tip>
**Choose a mode**
...
Agent mode is ZenOpus’s autonomous execution mode, designed to implement changes directly in your project.

When you give ZenOpus a task, it takes ownership of execution end to end. It understands your intent, explores the codebase for context, applies changes across files, and resolves issues that appear during development. While it works, you can see progress through visible tasks and review the results before moving on.
...
Agent mode is active unless you switch to [Plan mode](/features/plan-mode).

To use it, describe what you want and send your message. Clear requirements and constraints help ZenOpus produce better results.
...
While ZenOpus is working, tasks appear in the chat interface showing:
...
ZenOpus **processes one task at a time**. While ZenOpus is working, you can continue sending prompts and they will be added to a visible queue above the chat input.
...
When using Agent mode, ZenOpus does more than ensure code compiles.
...
ZenOpus has access to verification tools such as browser testing, frontend tests, and edge function verification to help understand system behavior, validate user workflows, and ensure backend logic works as expected. Most of these tools run only when you ask for them.
...
Many requests cost less than one credit, while more complex tasks may cost more. You can view the cost of a message from the three-dot menu on that message. See [credit usage](/introduction/plans-and-credits#credit-usage) for more information.

## FAQ

<AccordionGroup>
<Accordion title="Does ZenOpus modify my project directly when in Agent mode?">
Yes. ZenOpus applies changes directly to your project when using Agent mode. All modifications are visible through file diffs and summaries.
</Accordion>
<Accordion title="What happens if errors occur during implementation?">
ZenOpus can inspect logs, runtime output, and network activity and iterate on fixes until the issue is resolved or clarified. See [Test and verify your app](/features/testing) for more information.
</Accordion>
<Accordion title="Can I see what ZenOpus is doing while it works?">
Yes. Visible tasks show each step of execution, including progress and which files are being modified.
</Accordion>
<Accordion title="Can multiple messages be sent while working in Agent mode?">
...
Click the **stop button** while ZenOpus is responding to immediately halt the current task. This lets you stop a request to provide more context or correct the agent.

ZenOpus keeps all changes made up to that point, so you won’t lose completed work. Stopped requests are charged based on the work completed so far.
